...
if (isset($_GET['accept_id'])) {
  ...
  if ($row) {
    ...
    $sql = "INSERT INTO approved_request (pid,req_id, name, mobile_number, blood_group, birth_date, gender, weight, location, landmarks, req_form_path) 
                    VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iisssssssss", $row['pid'], $row['req_id'], $row['name'], $row['mobile_number'], $row['blood_group'], $row['birth_date'], $row['gender'], $row['weight'], $row['location'], $row['landmarks'], $row['req_form_path']);
    $stmt->execute();

    $sql = "UPDATE blood_req SET status = 'Accepted' WHERE req_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $accept_id);
    $stmt->execute();
                        ...
                        while ($row = $result->fetch_assoc()) {
                          ...
                          echo "
                            <tr>
                              ...
                              <td>
                              <a class='btn btn-success btn-rounded btn-fw' href='#' onclick=\"confirmAccept({$row['req_id']})\">Accept</a>

                              <a class = 'btn btn-danger btn-rounded btn-fw' href='#' onclick='confirmDelete($row[req_id])'>Decline</a>                             
                               </td>
                              </tr>
                              ";
                        }
        ...
        if (willDelete) {
            setTimeout(() => {
                swal({
                    ...
                    window.location.href = 'decline_request.php?accept_id=' + req_id;
                });
            }, 500);
        } else {
            swal({
