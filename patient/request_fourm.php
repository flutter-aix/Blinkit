                                    ...
                                    <div class="card">
                                        <div class="card-body">
                                            ...
                                            <form method="POST" enctype="multipart/form-data" class="forms-sample">
                                                ...
                                                <div class="form-group">
                                                    <label for="blood_quantity" class="form-label">Blood Quantity(in ml)</label>
                                                    <input type="number" class="form-control" id="location" name="blood_quantity">
                                                </div>
                                                <div class="form-group">
...
if (isset($_POST['submit']) && isset($_FILES['req_form'])) {
    ...
    if ($req_form != "") {
        ...
        if (move_uploaded_file($req_form_tmp, $uploadFilePath)) {
            $query = "SELECT * FROM blood_req WHERE mobile_number = ?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("s", $number);
            $stmt->execute();
            $result = $stmt->get_result();
            $count_number = $result->num_rows;

            if ($count_number == 0) {
                $query = "INSERT INTO blood_req(pid, name, mobile_number, blood_group, blood_quantity, birth_date, gender, weight, location, landmarks, message, req_form_path, status) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Pending')";

      $stmt = $conn->prepare($query);
      $stmt->bind_param("isssisssssss", $pid, $name, $number, $blood_group, $blood_quantity, $dob, $gender, $weight, $location, $landmarks, $message, $uploadFilePath);
      $result = $stmt->execute();
                if ($result) {
                    echo "<script>
                    Swal.fire({
                      title: ' Requested Succcessfully!',
                      icon: 'success'
                    });
                  </script>";
                } else {
